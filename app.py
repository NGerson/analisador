# =================================================================
# ARQUIVO app.py - VERSÃO FINAL, BLINDADA CONTRA ERROS
# =================================================================
import os
import requests
import random
...
# Configuração de logging para ver erros no Render
...
def atualizar_cache_estatisticas():
    global stats_cache
    if not API_KEY:
        app.logger.warning("TAREFA DE CACHE: Chave de API não encontrada.")
        return
    app.logger.info(f"TAREFA DE CACHE: Iniciando atualização para {len(LIGAS_PARA_BUSCAR)} ligas...")
    ...
    for nome_liga, id_liga in LIGAS_PARA_BUSCAR.items():
        try:
            ...
            if dados_api:
                ...
                app.logger.info(f"TAREFA DE CACHE: Cache para {nome_liga.upper()} atualizado.")
            else:
                app.logger.warning(f"TAREFA DE CACHE: A API não retornou dados para a liga {nome_liga.upper()}.")
        except Exception as e:
            app.logger.error(f"TAREFA DE CACHE: Erro para a liga {nome_liga.upper()}: {e}")
...
def analisar_jogo_api():
    try:
        ...
        if not dados:
            return jsonify({"erro": "Requisição JSON vazia ou malformada."}), 400
        ...
        if not stats_casa or not stats_fora:
            ...
            return jsonify({"erro": f"Um dos times não foi encontrado no cache para esta liga. Verifique a ortografia. Ex: {times_disponiveis}..."})
        ...
        app.logger.error(f"ERRO CRÍTICO NA ROTA /analisar: {e}", exc_info=True)
