# ARQUIVO servidor.py - ESTRUTURA REVISADA E ROBUSTA
...
def create_app():
    ...
    Toda a lógica está dentro desta função para evitar execução na importação.
    """
...
# --- FUNÇÕES AUXILIARES (FORA DA FUNÇÃO create_app) ---

def processar_dados_reais(stats_casa, stats_fora, nome_casa, nome_fora):
    # (Esta função permanece exatamente a mesma de antes)
    tips = []
    try:
    ...
    if tendencia_gols > 1.4: tips.append({"mercado": "Gols", "entrada": "Mais de 2.5 gols", "justificativa": f"Média de gols combinada alta ({tendencia_gols:.2f} por jogo).", "confianca": f"{int(65 + tendencia_gols * 10)}%"})
    else: tips.append({"mercado": "Gols", "entrada": "Menos de 2.5 gols", "justificativa": f"Média de gols combinada baixa ({tendencia_gols:.2f} por jogo).", "confianca": f"{int(80 - tendencia_gols * 10)}%"})
    diferenca_poder = (gols_pro_casa - gols_contra_casa) - (gols_pro_fora - gols_contra_fora)
    if diferenca_poder > 0.3: tips.append({"mercado": "Handicap Asiático", "entrada": f"{nome_casa} -0.5", "justificativa": "Time da casa possui um saldo de gols superior.", "confianca": f"{int(60 + diferenca_poder * 15)}%"})
    elif diferenca_poder < -0.3: tips.append({"mercado": "Handicap Asiático", "entrada": f"{nome_fora} -0.5", "justificativa": "Time visitante possui um saldo de gols superior.", "confianca": f"{int(60 + abs(diferenca_poder) * 15)}%"})
    tendencia_cartoes = cartoes_amarelos_casa + cartoes_amarelos_fora
    if tendencia_cartoes > 4.5: tips.append({"mercado": "Cartões", "entrada": "Mais de 4.5 cartões", "justificativa": f"Média combinada de cartões é alta ({tendencia_cartoes:.2f}).", "confianca": f"{int(60 + tendencia_cartoes * 5)}%"})
    media_escanteios_total = random.uniform(8.5, 12.0)
    if media_escanteios_total > 10.0: tips.append({"mercado": "Escanteios", "entrada": "Mais de 9.5", "justificativa": "Simulação indica times que atacam pelas laterais.", "confianca": f"{random.randint(70, 85)}%"})
    if not tips: return {"erro": "Não foi possível gerar uma análise com os dados obtidos."}
...
# A variável 'app' é criada aqui, chamando a função.
app = create_app()
